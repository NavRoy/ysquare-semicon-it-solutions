<?php
/* ============================================================
   career-submit.php — YSQUARE Career Application Backend
   Accepts POST only. Returns JSON { success, refId, error }.
   Reuses config.php (same one used by the internship form) —
   just add VALID_POSITIONS to config.php, or this file will
   fall back to the defaults below if it isn't defined there.
   ============================================================ */
...
/* ── Fallbacks (only used if not already defined in config.php) ── */
if (!defined('CAREER_REF_PREFIX')) {
    define('CAREER_REF_PREFIX', 'YSQ-CAR-' . date('Y') . '-');
}
if (!defined('VALID_POSITIONS')) {
...
if (!defined('VALID_EXPERIENCE')) {
    define('VALID_EXPERIENCE', ['Fresher', '0-1 Years', '1-3 Years', '3-5 Years', '5+ Years']);
}
...
$portfolio   = filter_var(trim($_POST['portfolio'] ?? ''), FILTER_SANITIZE_URL);
...
if (!in_array($position, VALID_POSITIONS))    $errors[] = 'Please select a valid position.';
if (!in_array($experience, VALID_EXPERIENCE)) $errors[] = 'Please select a valid experience level.';
if (!$location)                               $errors[] = 'Current location is required.';
if ($portfolio && !filter_var($portfolio, FILTER_VALIDATE_URL)) $errors[] = 'Portfolio / LinkedIn URL is not valid.';
if (strlen($motivation) < 20)                 $errors[] = 'Please tell us more about why you are a good fit (min 20 characters).';
...
if (empty($_FILES['resume']['name'])) {
    ...
    elseif (!in_array($mimeType, ALLOWED_TYPES))    $errors[] = 'File type not permitted.';
        ...
        $savedName = 'career_' . $safeName . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
...
$refId = CAREER_REF_PREFIX . mt_rand(1000, 9999);
...
/* ── HR Email (HTML) ────────────────────────────────────── */
$hrSubject = "New Career Application — {$fullName} · {$position} [{$refId}]";
...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="row"><div class="lbl">Position</div><div class="val">{$position}</div></div>
    ...
    <div class="row"><div class="lbl">Portfolio</div><div class="val">{$portfolio}</div></div>
    ...
    <div class="note"><strong>Why they're a fit:</strong><br>{$motivation}</div>
  </div>
...
$ackSubject = "Application Received — {$refId} | YSQUARE Careers";
...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="para">
      Thank you for applying to the <strong>{$position}</strong> role at YSQUARE. We have received your application and our HR team will personally review it within <strong>48 business hours</strong>.
    </div>
    <div class="para">
    ...
    <div class="summary">
      ...
      <div class="srow"><div class="slbl">Position</div><div class="sval">{$position}</div></div>
      <div class="srow"><div class="slbl">Experience</div><div class="sval">{$experience}</div></div>
